...
# Wurde mindestens ein Argument übergeben?
if [ $# -eq 0 ]; then
    echo "usage: $(basename $0) command" 1>&2
    exit 1
fi

# Logfile bestimmen und prüfen
logdir=${LOGDIR:-$HOME}
logfile=${logdir}/$(basename -s .sh $0).log
if ! touch $logfile 2>/dev/null; then
    echo "error: cannot create $logfile" 1>&2
    exit 2
fi

if ! [ -w $logfile 2>/dev/null ]; then
...
result=$?
logline1="$(date +'%Y.%m.%d %H:%M:%S') $command"
if [ $result -eq 0 ]; then
    echo "$logline1 SUCCESS" >> $logfile
else
    echo "$logline1 FAILURE Exit-Code: $result" >> $logfile
fi
echo "$output" >> $logfile
exit 0
